...
export const addServerThumbnail = createAction(ADD_SERVER_THUMBNAIL, api.uploadImage);
export const setPrevServerThumbnail = createAction(SET_PREV_SERVER_THUMBNAIL);
export const setPrevServerThumbnailList = createAction(SET_PREV_SERVER_THUMBNAIL_LIST);
...
export default handleActions({
    ...
    [INITIALIZE_INPUT]: (state, action) => initialState,
    ...pender({
        type: UPLOAD_IMAGE,
    ...
    [SET_PREV_SERVER_THUMBNAIL]: (state, action) => {
        return state.set('prevServerThumbnail', state.get('serverThumbnail'));

    },
    [SET_PREV_SERVER_THUMBNAIL_LIST]: (state, action) => {
        return state.update('prevServerThumbnailList', value => value.concat(state.get('prevServerThumbnail')));
    ...
    [FILTER_SERVER_FILES]: (state, action) => {
        ...
        let filtered = state.get('serverFiles').filter(
            (file, i) => {
                return file.id !== removed;
            }
        );

        return state.set('serverFiles', filtered)
            .update('willRemoveServerFiles', value => value.concat(removed));
    },
    [FILTER_SERVER_THUMBNAIL]: (state, action) => {
        const { removed } = action.payload;
        return state.set('willRemoveThumbnailFile', removed)
    ...
    ...pender({
        ...
        onSuccess: (state, action) => {
            const { storedFileName, localFileName } = action.payload.data;
            return state.update('serverFiles', value => value.concat({ id: storedFileName, localFileName }));
        }
    }),
    ...pender({
    ...
    [SET_SERVER_INPUTS]: (state, action) => {
        ...
        const { actors, endYear, genre, name, startYear, story, teasers, firstBroadcasted } = serverSeries;
        let actornameArr = actors.map(
            (actor, i) => {
                return actor.name
            }
        );

        let videoIdArr = teasers.map(
            (teaser, i) => {
                return teaser.videoId
            }
        );

        let teasernameArr = teasers.map(
            (teaser, i) => {
                return teaser.name
            }
        );

        return state.setIn(['inputs', 'name'], name)
